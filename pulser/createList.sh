...
then
  ...
  DATAPATH=/mnt/spirit/rawdata/misc/pulser
fi
/mnt/spirit/rawdata/misc/pulser/run3222/cobo08/run_3222.dat.01-06-16_10h32m12s
...
do
  ...
  ls $DATAPATH/run$RUN/cobo$COBO/run_$RUN*s >> $OUTPUT
  for FILENUM in {1..100};
  do
    #RESULT=`ls $DATAPATH/spdaq0?/narval/cobo$COBO/run_$RUN*s.$FILENUM 2> /dev/null`
    RESULT=`ls $DATAPATH/run$RUN/cobo$COBO/run_$RUN*s.$FILENUM 2> /dev/null`
    if [[ -a $RESULT ]];
    then
      echo $RESULT >> $OUTPUT
    else
