...
(
 ...
 TString fPathToData = "/mnt/spirit/analysis/user/leej/macros/hitcluster/data/",
...
)
{
  ...
  TString out = fPathToData+"run"+sRunNo+"_s"+sSplitNo+".reco."+version+".test1.root";
  ...
  } decoder -> SetGGNoiseData(fGGData); decoder -> SetDataList(raw);
