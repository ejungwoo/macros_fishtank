...
TH2F* hdzvsX = new TH2F("hdzvsX","Tpc dz vs X",1000,-170.,170.,1000,-20000.,20000.);
...
TH2F* hdtvsT = new TH2F("hdtvsT","Tof vs Mom in",1000,-3000.,3000.,1000,0.,200.);
...
TH2F* hXtvXn = new TH2F("hXtvXn","x tpc vs x neuland",100,-1,1,100,-150,150);



void loop(TChain *fChain, TChain *fChainx, int runNo)
{
  //TFile *outFile= new TFile("NLout.root","recreate");

  TFile *outFile= new TFile(Form("./NLdata/NLrun%i.root",runNo),"recreate");
  
  auto NLtree = new TTree("NL","");//tree to store simple information from neuland
  Double_t NLx,NLy;
  Double_t NLprotonx,NLprotony;
  
  Int_t NLrun, NLevent;
  NLtree -> Branch("NLx",&NLx);
  NLtree -> Branch("NLy",&NLy);
  NLtree -> Branch("NLprotonx",&NLprotonx);
  NLtree -> Branch("NLprotony",&NLprotony);
  NLtree -> Branch("NLrun",&NLrun);
  NLtree -> Branch("NLevent",&NLevent);
	   ...
	   while (jentry+1 == current_event) {
		 ...
		 if (z1->IsInside(clusters[i].etot,clusters[i].tf)) {
 		 ...
 		 if (protons->IsInside(clusters[i].etot,clusters[i].tf)) {
		   ...
		   NLprotonx=clusters[i].xf;
		   NLprotony=clusters[i].yf;
		 ...
		 NLx=clusters[i].xf;
		 NLy=clusters[i].yf;
		 hXtvXn->Fill(dx,clusters[i].xf);
		 
		 }
		 
		 hdtvsT->Fill(mom*(dx*sin(30./57.3)+dz*cos(30./57.3)), clusters[i].tf);
  ...
  void neuland4()
{
  ...
  */
  for(int i=runs[0];i<=runs[numData-1];i++) {  //  EOS2 all  2839 - 3166
  ...
  printf("kraaaaj\n");
  /*
  auto cj=new TCanvas("cj","",0,0,1200,800);
  cj->Divide(2,2);
  cj->cd(1);
  //hDYvsDX->Draw("colz");
  hXtvXn->Draw("colz");
