...
TH2F* hVTofvsEhitMatch2 = new TH2F("hVTofvsEhitMatch2","Veto Tof vs Ehit match2",200,0.,EhitRange,3000,0.,300.);
...
TH2F* hTsyncFine = new TH2F("hTsyncFine","Tsync Fine vs Bars",num_bars,0.5,num_bars+0.5,500,-10,10);
TH2F* hDT_dtfine = new TH2F("hDT_dfine","DT fine vs Bars",num_bars,0.5,num_bars+0.5,500,-10,10);
...
TH2F* hclyz = new TH2F("hclyz","yz",400,-200.,200.,400,-200,200);
...
TH2F* hNLvsVA=new TH2F("hNLvsVA","Multiplicity vs VA",35,-0.5,34.5,40,-0.5,39.5);
...
TH2F* hNLvsVAmix=new TH2F("hNLvsVAmix","Multiplicity vs VA mix",35,-0.5,34.5,40,-0.5,39.5);
...
TH2F* hNLvsVAmix1=new TH2F("hNLvsVAmix1","Multiplicity vs VA mix",35,-0.5,34.5,40,-0.5,39.5);
...
TH2F* hNLvsVAdiff=new TH2F("hNLvsVAdiff","Multiplicity vs VA diff",35,-0.5,34.5,40,-0.5,39.5);
...
TH2F* hNLvsVAdiff1=new TH2F("hNLvsVAdiff1","Multiplicity vs VA diff",35,-0.5,34.5,40,-0.5,39.5);
...
void loop(TChain *fChain)
{
  ...
  for(int i=1;i<=2*num_planes;i++){
      ...
      sprintf(name2,"Xhit vs Yhit clust %i",i);     
      hPlanecut[i] = new TH2F(name1,name2,68,-170,170,680,-170,170); 
  }
  for(int i=1;i<=2*num_planes;i++){
      sprintf(name1,"plane%iacut",i);
      sprintf(name2,"Xhit vs Yhit clust %i",i);     
      hPlaneacut[i] = new TH2F(name1,name2,68,-170,170,680,-170,170); 
  }
  ...
  for(int i=0;i<=num_bars;i++){
    ...
    hTofbar[i] = new TH1F(name1,name2,100,36.,39.); // was 2000 bins
  }
  for(int i=0;i<=8;i++){
    sprintf(name1,"Tofplane%i",i);
    sprintf(name2,"Gamma tof of plane %i",i);     
    hTofplane[i] = new TH1F(name1,name2,100,36.,39.); // was 2000 bins
  }
  for(int i=0;i<=num_bars;i++){
...
//      }
   ...
   if (clno>0) {
	 ...
	 if(fabs(clusters[i].xf/(Distance+clusters[i].zf)-clusters[j].xf/(Distance+clusters[j].zf))<0.02&&
	    fabs(clusters[i].yf/(Distance+clusters[i].zf)-clusters[j].yf/(Distance+clusters[j].zf))<0.02&&
	    fabs(clusters[i].zl-clusters[j].zf)<12.5) {
	 ...
	 if (in_veto==0) { // ############## NEUTRAL CLUSTERS ####################
	   ...
	   //if (clusters[i].tf>1483.13/(pow(clusters[i].etot,0.757556)-0.6278)+37.9272) {
	   ...
	   hTofvsBarClustNoVeto->Fill(cl_bar,clusters[i].etot);
	   ...
	   for(int j=0;j<nlv_nbar;j++) {
	     ...
	     if (nlv_tcal0[j]>0.&&nlv_tcal1[j]>0.) {
	       ...
	       //if (fabs(0.5*sqrt(nlv_qcal0[j]*nlv_qcal1[j])+1.5-(clusters[i].tf-(nlv_tcal0[j]+nlv_tcal1[j])/2.))<1.&&
	       //  fabs(sqrt(nlv_qcal0[j]*nlv_qcal1[j])-7.)<5) {
	       hTofvsEhitClustnv[(cl_bar-1)/50+1]->Fill(clusters[i].etot,clusters[i].tf);
	       //}

	       if (fabs(0.5*sqrt(nlv_qcal0[j]*nlv_qcal1[j])-1.-(clusters[i].tf-(nlv_tcal0[j]+nlv_tcal1[j])/2.))<1.&&
		   fabs(sqrt(nlv_qcal0[j]*nlv_qcal1[j])-7.)<5) {

		 hVTofvsEhitMatch2->Fill(clusters[i].etot,clusters[i].tf);
   ...
   Double_t tbefore, tbefore2, tafter, tafter2;
   ...
   for (iBar=1;iBar<=num_bars;iBar++) {
	 ...
	 if (fired[jBar]==2&&jBar!=iBar) {
	   ...
	   if (fabs(xhit[iBar]-xhit[jBar])<7.5&&
	       fabs(yhit[iBar]-yhit[jBar])<7.5&&
	       zhit[jBar]==zhit[iBar]+10.) tafter2 = thit[jBar];
	   if (fabs(xhit[iBar]-xhit[jBar])<7.5&&
	       fabs(yhit[iBar]-yhit[jBar])<7.5&&
	       zhit[jBar]==zhit[iBar]+5.) tafter = thit[jBar];
	   if (fabs(xhit[iBar]-xhit[jBar])<7.5&&
	       fabs(yhit[iBar]-yhit[jBar])<7.5&&
	       zhit[jBar]==zhit[iBar]-5.) tbefore = thit[jBar];
	   if (fabs(xhit[iBar]-xhit[jBar])<7.5&&
	       fabs(yhit[iBar]-yhit[jBar])<7.5&&
	       zhit[jBar]==zhit[iBar]-10.) tbefore2 = thit[jBar];
	 }
       }

       if (tbefore>-10000.&&tafter>-10000.) {
	 //hTsyncFine->Fill(iBar,thit[iBar]-(tbefore+tafter)/2.);
	 hTsyncFine->Fill(iBar,thit[iBar]-tbefore);
	 hTsyncFine->Fill(iBar,thit[iBar]-tafter);
       }

       if (iBar<=50&&tafter>-10000.&&tafter2>-10000.) {
	 //hTsyncFine->Fill(iBar,thit[iBar]-(2.*tafter-tafter2));
	 hTsyncFine->Fill(iBar,thit[iBar]-tafter);
	 hTsyncFine->Fill(iBar,thit[iBar]-tafter2);

       }

       if (iBar>=350&&tbefore2>-10000.&&tbefore>-10000.) {
       	 //hTsyncFine->Fill(iBar,thit[iBar]-(2.*tbefore-tbefore2));
       	 hTsyncFine->Fill(iBar,thit[iBar]-tbefore);
       	 hTsyncFine->Fill(iBar,thit[iBar]-tbefore2);
       }

       if(mult>0) { //&&(fabs(a)>1.0||fabs(ah)>1.0)) {
	 if (ehit[iBar]>0.) {
	 ...
	 for (jBar=1;jBar<=num_bars;jBar++) {
	   if ((fired[jBar]==2)&&(jBar!=iBar)) {
	     if (ehit[iBar]>0.&&ehit[jBar]>0.) {
	       ...
	       // if((abs(iBar-jBar)%100)==0&&((iBar-1)%100)<50) {      // cosmics
	       if((iBar-1)/50==(jBar-1)/50) {                          //beam
		 ...
		 //dist = (yhit[iBar]-yhit[jBar])/fabs(yhit[iBar]-yhit[jBar])*  // cosmics
		 dist = -1.*(zhit[iBar]-zhit[jBar])/fabs(zhit[iBar]-zhit[jBar])*  // beam
	       ...
	       if (jBar<=50) {
		 hDT_dtfine->Fill(jBar,thit[jBar]-thit[75]);
		 hDT_dtfine->Fill(jBar,thit[jBar]-thit[76]);
	       } else {
		 hDT_dtfine->Fill(jBar,thit[jBar]-thit[((jBar-1)/50-1)*50+25]);
		 hDT_dtfine->Fill(jBar,thit[jBar]-thit[((jBar-1)/50-1)*50+26]);
	       }
   ...
   hNLvsKA->Fill(katmult[evt_cnt],nclno);
   hNLvsKY->Fill(kyomult[evt_cnt],nclno);
   hNLvsVA->Fill(ampmult[evt_cnt],nclno);
   hKYvsKA->Fill(katmult[evt_cnt],kyomult[evt_cnt]);
   hNLvsKAmix->Fill(katmult[evt_cnt+1],nclno);
   hNLvsKYmix->Fill(kyomult[evt_cnt+1],nclno);
   hNLvsVAmix->Fill(ampmult[evt_cnt+1],nclno);
   hKYvsKAmix->Fill(katmult[evt_cnt+1],kyomult[evt_cnt+1]);
   hNLvsKAmix1->Fill(katmult[evt_cnt+2],nclno);
   hNLvsKYmix1->Fill(kyomult[evt_cnt+2],nclno);
   hNLvsVAmix1->Fill(ampmult[evt_cnt+2],nclno);
...
void calibrate_tdiff3()
{
  ...
  for(int j=1;j<=num_bars;j++) {
    ...
    for(int i=1;i<nbins;i++) {
      TH1F *cch;
      cch = (TH1F*)hTdiffvsXBar[j]->ProjectionY("cch",i,i);
      ntot = cch->GetEntries();
...
void calibrate_tsync_dtfine()
{

  Int_t ntot, peakbin, nbins;
  ofstream a_file ( "./parameter_test/neuland_dtfine.txt" );
  a_file<<"#bar  ediff_offset[ns]  esync_offset[ns]  att[cm/ns] \n";

  Double_t y;
  
  nbins=hTsyncFine->GetNbinsX(); 

  if (nbins!=num_bars) printf("Impossible\n");
 
  for(int i=1;i<=nbins;i++) {
    TH1F *cch;
    cch = (TH1F*)hTsyncFine->ProjectionY("cch",i,i);
    ntot = cch->GetEntries();
    ...
    if (ntot>0) {
      ...
      TF1 *f1 = new TF1("f1", "gaus", y-1.0, y+1.0);
      cch->Fit("f1", "R");
      y = f1->GetParameter(1); //value of 1st parameter	
  ...
  for(int i=1;i<=nbins;i++) {
    cout<< i <<"     " << tdiff[i] <<"     " << tsync[i]<< "     "<< vscint[i] << "\n";
  }  
  a_file.close();
}

void calibrate_tsync_dtdtfine()
{

  Int_t ntot, peakbin, nbins;
  ofstream a_file ( "./parameter_test/neuland_dtdtfine.txt" );
  a_file<<"#bar  ediff_offset[ns]  esync_offset[ns]  att[cm/ns] \n";

  Double_t y;
  
  nbins=hDT_dtfine->GetNbinsX(); 

  if (nbins!=num_bars) printf("Impossible\n");
 
  for(int i=1;i<=nbins;i++) {
    TH1F *cch;
    cch = (TH1F*)hDT_dtfine->ProjectionY("cch",i,i);
    ntot = cch->GetEntries();
...
void calibrate_ediff3()
{
  ...
  for(int j=1;j<=num_bars;j++) {
    ...
    for(int i=1;i<nbins;i++) {
      TH1F *cch;
      cch = (TH1F*)hEdiffvsXBar[j]->ProjectionY("cch",i,i);
      ntot = cch->GetEntries();
...
void calibrate_walk()
{
  ...
  for(int i=1;i<nbins;i++) {
    TH1F *cch;
    cch = (TH1F*)hTofvsErawall->ProjectionY("cch",i,i);
    ntot = cch->GetEntries();
...
void calibrate_esync2()
{
  ...
  for(int i=1;i<nbinsh;i++) {
    TH1F *cch;
    cch = (TH1F*)hEdepvsPathH->ProjectionY("cch",i,i);
  ...
  for(int i=1;i<nbins;i++) {
    ...
    ccc = (TH1F*)hEdepvsPathV->ProjectionY("ccc",i,i);
    ntot = ccc->GetEntries();
...
void subhisto()
{
  ...
  nx = hNLvsVA->GetXaxis()->GetNbins(); 
  ny = hNLvsVAmix->GetYaxis()->GetNbins(); 
  for (int i=1;i<=nx;i++) { 
    for (int j=1;j<=ny;j++) { 
      c1 =hNLvsVA->GetBinContent(i,j); 
      if (c1 > 0) { 
	c2 =hNLvsVAmix->GetBinContent(i,j); 
	//if (c1-c2 > 0) 
	  hNLvsVAdiff->SetBinContent(i,j,c1-1.0*c2); 
      }
    }
  }
  ...
  nx = hNLvsVAmix1->GetXaxis()->GetNbins(); 
  ny = hNLvsVAmix->GetYaxis()->GetNbins(); 
  for (int i=1;i<=nx;i++) { 
    for (int j=1;j<=ny;j++) { 
      c1 =hNLvsVAmix1->GetBinContent(i,j); 
      if (c1 > 0) { 
	c2 =hNLvsVAmix->GetBinContent(i,j); 
	//if (c1-c2 > 0) 
	  hNLvsVAdiff1->SetBinContent(i,j,c1-1.0*c2); 
      }
    }
  }
